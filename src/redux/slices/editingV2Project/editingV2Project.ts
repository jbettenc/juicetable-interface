...
const editingV2ProjectSlice = createSlice({
  ...
  reducers: {
    ...
    setFundingCycleMetadata: (
      ...
      state.fundingCycleMetadata = {
        ...action.payload,
        tokenUnlockingSchedules:
          state.fundingCycleMetadata.tokenUnlockingSchedules,
        tokenName: state.fundingCycleMetadata.tokenName,
        tokenSymbol: state.fundingCycleMetadata.tokenSymbol,
      }
    },
    setTokenUnlockingSchedule: (
      state,
      action: PayloadAction<any[] | undefined>,
    ) => {
      state.fundingCycleMetadata.tokenUnlockingSchedules = JSON.stringify(
        action.payload ?? [],
      )
    },
    setTokenName: (state, action: PayloadAction<string>) => {
      state.fundingCycleMetadata.tokenName = action.payload
    },
    setTokenSymbol: (state, action: PayloadAction<string>) => {
      state.fundingCycleMetadata.tokenSymbol = action.payload
    },
