...
> = ({ isDefocused, isSelected, isDisabled, children }) => {
  const borderColorClassNames = useMemo(() => {
    if (isSelected) return 'border-orange-500'
    return classNames(
      !isDisabled ? 'hover:border-smoke-500' : '',
      isDefocused ? 'border-smoke-200' : 'border-smoke-300',
    )
  }, [isDefocused, isDisabled, isSelected])

  const backgroundColorClassNames = useMemo(() => {
    if (isDefocused) return 'bg-white'
    if (isSelected) return 'bg-orange-500/10'
  }, [isDefocused, isSelected])
...
> = ({
  ...
  const titleColorClassNames = useMemo(() => {
    if (defocused) return 'text-black'
